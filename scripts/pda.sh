...
load_pda_config() {
  ...
  [[ -f "$apps_file" ]] || die "Arquivo apps.json nao encontrado: $apps_file"
...
clone_or_update_repo() {
  ...
  if [[ -d "$target_dir/.git" ]]; then
    log_info "Atualizando repositorio PDA em $target_dir."
    git -C "$target_dir" pull --ff-only
  else
    log_info "Clonando repositorio PDA em $target_dir."
...
install_app() {
  ...
  if [[ -n "$check_command" ]] && command_exists "$check_command"; then
    log_success "Aplicativo PDA ja instalado: $name"
    return 0
  fi
...
install_apps_from_json() {
  ...
  for ((index = 0; index < apps_length; index++)); do
    ...
    [[ "$name" != "null" && -n "$name" ]] || die "Aplicativo PDA sem nome no indice $index."
    [[ "$install_command" != "null" && -n "$install_command" ]] || die "Aplicativo PDA sem comando de instalacao: $name."
